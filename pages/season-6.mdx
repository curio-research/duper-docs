# Season VI

Dates: September 1, 2025 to November 15, 2025 (2.5 months), followed by Duper Bowl.

## 🎉 New?

- 📱 **Official App Store + Google Play launch** in the US
- 🚀 **Beginner Journey**: Enhanced onboarding for first-time players, including 3 new interactive tutorials
- 💰 **Casual Mode Update**: Entry increased to 2 Chips up from 1 Chip
- ✨ **Sparks Utility**: Use Sparks to extend turn time and unlock milestones
- 🗿 **Coming Soon**: Custom in-game stickers

<img src="/ingamesticker.png" alt="In-game sticker preview" width="360" />

## 🎁 For New Players

If you're new to Duper, welcome! Check out [Quick Start](https://docs.duper.gg/) for a quick overview.

### Welcome Bonuses

- You start with **2 two-chip vouchers** (worth $4 USD total) which expire in 2 months
- 100 Sparks for all new accounts
- **Beginner Journey**: Complete the tutorials and play 3 casual games to earn additional **2 two-chip vouchers and 1 one-chip voucher** (worth $5 total)
- **Special offer** for first-time deposit: we'll match up to $10 USD of your first deposit, so you'll double your deposit!

### 🔥 First-Time Deposit Promo

For all new users, we will double your first-time deposit up to 10 chips.

Examples:

- Deposit 4 usd → play with 8 Chips
- Deposit 10 usd → play with 20 Chips (max match)

Offer valid once per account during Season VI.

## 🎫 Season VI Pass

A Season Pass unlocks most features and rewards for the season. Great for getting the full experience.

### Pass Options

- 🥈 Silver Pass: $30
  - Receive up to $113.5 worth of direct rewards (not including potential game earnings)
  - Perfect for casual players
- 🥇 Gold Pass: $80
  - Receive up to $360 worth of direct rewards (not including potential game earnings)
  - Best value for committed players

### Pass Benefits

- 💰 Instant cashback
  - Silver: $30 in Chip vouchers
  - Gold: $80 in Chip vouchers
  - Vouchers expire after 1 month
- 🏆 Milestone rewards (more for Gold Pass)
  - Gold Keys
  - Chip vouchers
  - Emblems
  - Sparks
- 🎲 Loss subsidy raffles
  - Claimable every 2-loss streak in Ranked/Casual
  - Get Chips vouchers or Sparks
- 🦄 Quests & daily claims
- 😎 In-game chat stickers (Gold Pass only)

### Upgrade Path

You can upgrade from Silver to Gold for $50 at any time and receive:

- 20 one-chip vouchers, 5 two-chip vouchers, and 4 five-chip voucher

See full details on pass benefits on the [pass store page(opens in a new tab)](https://www.duper.gg/store?id=season-pass).

## 🎮 Economy Highlights

### Casual Mode Changes

- Entry fee: **2 Chips** ($2), increased from previous 1 Chip
- Available 24/7
- AI bots may join to speed up matchmaking

### Sparks Utility

- Gold Key: 999 Sparks
- Emblems: Trade for 100 Sparks each
- **Unlock bonus milestone rewards in season pass**
- **Extend turn time**

## 🥣 Duper League & Duper Bowl

The Duper League and Duper Bowl will be back! Play and get qualified for massive prizes.

Check out the [tournament page](./tournaments) for more details.
