...
In both cases, all of the player’s tiles except the capital tile permanently disappear from the session, along with all units on those tiles. Such a player exit usually shifts the diplomatic situation quite a bit, and at the very least makes Tile Count Condition more attainable for others. The dead player enters spectator mode until the session ends.
