...
Every time a combo reward is won, the player also gains an opportunity to raise (with the option of not doing so) their entry fee. If the player decides to raise, they mint 3 additional gems, and each one costs 10% of the lowest pre-game bid. After each raise, the cost per raise increments by 10%.

> _Example: If the lowest bid is 100 Sparks (from Purple), the first time a player raises, they pay 3 gems \* 10% \* 100 min buy-in = 30 Sparks. Next time, the unit price of a gem rises to 20%. To mint 3 gems, the player will need to commit an additional 60 Sparks as entry fee._
